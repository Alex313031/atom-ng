  ...
  core: {
    ...
    properties: {
      ...
      titleBar: {
        ...
        enum: ['native', 'hidden'],
        description:
          'Experimental:  The title bar can  be completely `hidden`.<br>This setting will require a relaunch of Atom-ng to take effect.'
      },
      ...
      uriHandlerRegistration: {
        ...
        default: 'prompt',
        description:
          'When should Atom-ng register itself as the default handler for atom:// URIs?',
        enum: [
          {
            value: 'prompt',
            description:
              'Prompt to register Atom-ng as the default atom:// URI handler'
          },
          {
            value: 'always',
            description:
      ...
      themes: {
        ...
        description:
          'Names of UI and syntax themes which will be used when Atom-ng starts.'
      },
      audioBeep: {
        type: 'boolean',
        default: true,
      ...
      closeDeletedFileTabs: {
        ...
        description:
          'Close corresponding editors when a file is deleted outside Atom-ng.'
      },
      destroyEmptyPanes: {
        type: 'boolean',
        default: true,
      ...
      restorePreviousWindowsOnStart: {
        ...
        default: 'yes',
        description:
          "When selected 'no', a blank environment is loaded. When selected 'yes' and Atom-ng is started from the icon or `atom` by itself from the command line, restores the last state of all Atom-ng windows; otherwise a blank environment is loaded. When selected 'always', restores the last state of all Atom-ng windows always, no matter how Atom-ng is started."
      },
      ...
      automaticallyUpdate: {
        description:
          'Automatically update Atom-ng when a new release is available.',
        type: 'boolean',
        default: false
      },
      ...
      fileSystemWatcher: {
        description:
          'Choose the underlying implementation used to watch for filesystem changes. Emulating changes will miss any events caused by applications other than Atom-ng, but may help prevent crashes or freezes.',
        type: 'string',
        default: 'native',
        enum: [
          {
          ...
          {
            value: 'atom',
            description: 'Emulated with Atom-ng events'
          }
        ]
      },
      ...
      colorProfile: {
        description:
          "Specify whether Atom-ng should use the operating system's color profile (recommended) or an alternative color profile.<br>Changing this setting will require a relaunch of Atom-ng to take effect.",
        type: 'string',
        default: 'default',
        enum: [
          {
  ...
  editor: {
    ...
    properties: {
      ...
      invisibles: {
        type: 'object',
        description:
          'A hash of characters Atom-ng will use to render whitespace characters. Keys are whitespace character types, values are rendered characters (use value false to turn off individual whitespace character types).',
        properties: {
          eol: {
            type: ['boolean', 'string'],
            default: '¬',
...
if (process.platform === 'darwin') {
  configSchema.core.properties.titleBar = {
    ...
    description:
      'Experimental: A `custom` title bar adapts to theme colors. Choosing `custom-inset` adds a bit more padding. The title bar can also be completely `hidden`.<br>Note: Switching to a custom or hidden title bar will compromise some functionality.<br>This setting will require a relaunch of Atom-ng to take effect.'
  };
